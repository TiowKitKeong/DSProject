...
selected = option_menu(
    menu_title=None,
    options=["Stock Predictions", "Technical Indicators", "Returns", "EDA", "About & Credits"],
    icons=["calendar-week-fill", "bar-chart-fill", "currency-dollar", "search", "people-fill"],
...
if selected == "EDA":
    ...
    START = "2018-01-01"
    TODAY = date.today().strftime("%Y-%m-%d")
    selected_stock = st.selectbox('Select stock for EDA: ', stocks, index=1)
    ...
    profile_EDA = ProfileReport(data, explorative=True)

    "---"

    st.subheader("Exploratory Data Analysis")
    st_profile_report(profile_EDA)
...
if selected == "About & Credits":
    ...
    with col1:
        st.write("""
            ...
            \n**EDA**:                    Exploratory Data Analysis result on selected stock dataset
